package com.example.verticallayouttextviewlib;
...
public class VerticalLayoutTextView extends View {
    ...
    private boolean isLastCharEnglish(int i) {
        return i > 1 && isEnglishCharOrNumberChar(this.mText.charAt(i - 1));
    }
        ...
        while (i < this.mText.length()) {
            ...
            if (this.mLastCanShowCharIndex == 0 || i != this.mLastCanShowCharIndex) {
                ...
                if (ch == '\n') {
                    ...
                    if (i > 0 && isEnglishCharOrNumberChar(ch) && ((float) mTextPosY) + charHeight > ((float) this.mHeight)) {
                        if (isEnglishCharOrNumberChar(this.mText.charAt(i - 1))) {
    ...
    private int measureWidth() {
        ...
        while (i < contentLength) {
            ...
            if (ch == '\n') {
                ...
                boolean isEnglishChar = isEnglishCharOrNumberChar(ch);
    ...
    private boolean isSpecialChar(boolean lastCharIsEnglish, char ch) {
        return (ch >= 'a' && ch <= 'z') || ((ch >= 'A' && ch <= 'Z') || (ch >= '0' && ch <= '9') || ((lastCharIsEnglish && this.mTransAfterEngChars.contains(ch + "")) || this.mAlwaysTransChars.contains(ch + ""))
                || ((int) ch > 127 && (int) ch < 256));
    }

    private boolean isEnglishCharOrNumberChar(char ch) {
